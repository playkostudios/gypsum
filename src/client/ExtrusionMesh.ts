...
export interface ExtrusionOptions extends ExtrusionMaterialOptions {
    ...
    /**
     * The maximum smoothing angle for auto-smoothing. Default is close to 45
     * degrees.
     */
    maxSmoothAngle?: number;
    /**
     * The UVs for the starting base. Must either match the amount of points in
     * the base's polyline, or exceed by one if the UVs don't connect back.
     */
    startBaseUVs?: Array<vec2>;
    /**
     * The UVs for the end base. Must either match the amount of points in
     * the base's polyline, or exceed by one if the UVs don't connect back.
     */
    endBaseUVs?: Array<vec2>;
    /**
     * A tuple containing the configuration for the side (segment) UVs. The
     * first member contains the V value at the start of the extrusion (0 by
     * default). The second member contains the V value at the end of the
     * extrusion (1 by default). The third member contains a list of U values
     * for each point in the polyline, with an optional extra U value for the
     * wrap-around point; if null, then this will be automatically generated by
     * evenly distributing U values from 0 to 1 along each point in the polyline
     * with a wrap-around point.
     */
    segmentsUVs?: [startV: number | null, endV: number | null, segmentsUs: Array<number> | null];
    /**
     * The scale of each point in the curve. If omitted, then each curve point
 ...
 */
export class ExtrusionMesh extends MeshGroup {
    /**
